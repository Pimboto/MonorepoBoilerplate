...
import { Brush2 } from 'iconsax-reactjs';
import { useCallback } from 'react';
import type { SkyImageEditsParams } from '../types';

type SkyImageEditsNodeProps = NodeProps & {
  data: SkyImageEditsParams;
};

export function SkyImageEditsNode({ data, id, selected }: SkyImageEditsNodeProps) {
  const { setNodes } = useReactFlow();

  const handleChange = useCallback(
    (field: keyof SkyImageEditsParams, value: unknown) => {
      setNodes(nds =>
    ...
    >
      ...
      <div className="relative flex items-center gap-3 px-4 py-3 border-b border-divider">
        <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-default-200 dark:bg-default-100">
          <Brush2 size={16} variant="Bold" className="text-foreground" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-foreground truncate">Image Editor</h3>
          <p className="text-[10px] text-default-400 font-medium tracking-wide uppercase">Edits</p>
        </div>
      ...
      <div className="relative px-4 py-3 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-default-400">-1 = random</span>
          </div>
          <input
            type="number"
            value={data.seed}

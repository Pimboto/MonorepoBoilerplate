...
export const ThemeSwitch = ({ className, intensity = 1 }: ThemeSwitchProps) => {
  ...
  useEffect(() => {
    ...
    const fragSrc = `#version 300 es
      ...
      vec2 flow(vec2 p){
        float e=0.01;
        float n1=fbm(p); float nx=fbm(p+vec2(e,0.0)); float ny=fbm(p+vec2(0.0,e));
      ...
      void main(){
        ...
        vec3 tintLight=vec3(1.00,0.96,0.90);
        vec3 tintDark=vec3(0.86,0.92,1.00);
  ...
  return (
    <div
      className={clsx(
        'relative flex h-8 w-16 select-none items-center justify-center',
    ...
    >
      ...
      <button
        ...
        onClick={onToggle}
        className="absolute inset-0 h-full w-full rounded-full cursor-pointer bg-transparent border-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
        style={{ zIndex: 10 }}
      >
        {/* Thumb */}
        <span
          className={clsx(
            'absolute top-0.5 left-0.5 h-7 w-7 rounded-full bg-background/85 shadow-sm z-30',
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-[14px] -translate-x-1/2 z-20 flex items-center"
      >
        <Sun1
          size={14}
          variant="Bold"
          className={clsx(
            ...
            isDark ? 'opacity-45' : 'opacity-100 rotate-12',
          )}
        />
      </span>
      <span
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-[50px] -translate-x-1/2 z-20 flex items-center"
      >
        <Moon
          size={14}
          variant="Bold"
          className={clsx(
            'transition-all duration-300',
            isDark ? 'opacity-100 -rotate-12' : 'opacity-45',
          )}
        />
      </span>
    </div>
